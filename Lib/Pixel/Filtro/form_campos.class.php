...
{
    private $ValidaJS, $ValidaPHP, $ZIndex;
    ...
    public function retornaValor($Metodo, $Nome)
    {
        ...
        switch ($Metodo)
        {
            case "POST"   : return $_POST[$Nome];    break;
            case "GET"    : return $_GET[$Nome];     break;
            case "REQUEST": return $_REQUEST[$Nome]; break;
            case "SESSION": return $_SESSION[$Nome]; break;
            case "COOKIE" : return $_COOKIE[$Nome];  break;
            case "FILES"  : return $_FILES[$Nome];   break;
            default: return null;
        }
    }
    ...
    public function htmlInput($CFG, $Valor, $Type)
    {
        ...
        switch ($Type)
        {
            case "password" : $Tipo = "type=\"password\""; $ModFil = false; break;
            case "hidden"   : $Tipo = "type=\"hidden\"";   $ModFil = false; break;
            case "text"     : $Tipo = "type=\"text\"";     break;
            case "submit"   : $Tipo = "type=\"submit\"";   $ModFil = false; break;
            case "reset"    : $Tipo = "type=\"reset\"";    $ModFil = false; break;
            case "button"   : $Tipo = "type=\"button\"";   $ModFil = false; break;
            case "checkbox" : $Tipo = "type=\"checkbox\""; $ModFil = false; break;
            case "radio"    : $Tipo = "type=\"radio\"";    $ModFil = false; break;
            case "file"     : $Tipo = "type=\"file\"";     $ModFil = false; break;
            case "image"    : $Tipo = "type=\"image\"";    break;
            default         : $Tipo = "type=\"text\"";     $ModFil = false;
        }
        ...
        if($Type == "checkbox" or $Type == "radio")
        {
            ...
            if($Checked == $Nome or $CFG['Vale'] == $Valor) $Checado = "checked=\"checked\"";
        ...
        if($Conteiner != '')
        {
            return ($ModFil == true) ? "<div id=\"$Conteiner\">".$this->opFiltro($CFG,$Retorno)."</div>" : "<div id=\"$Conteiner\">".$Retorno."</div>";
        }
        else
        {
    ...
    public function htmlEditorInput($CFG, $Valor)
    {
        ...
        $Edit->BasePath	    = empty($_SESSION['UrlModuloBaseSite']) ? $_SESSION['UrlBase']."".$Pasta."/" : $_SESSION['UrlModuloBaseSite']."".$Pasta."/";
        $Edit->Width	    = $Largura;
        $Edit->Height	    = $Altura;
        $Edit->ToolbarSet   = $Ferramentas;
        $Edit->Value	    = $Valor;
        $Edit->Pasta	    = $Pasta;
        $Edit->InsertFuncao	    = $InsertFuncao;
    ...
    */
    ...
    {
        if($CFG['ProcessarComo'] == '') $CFG['ProcessarComo'] = "Texto";
    ...
    public function divEditorPopup($CFG){
        ...
        $Html = '
                ...
                <div class="editor_popup" style="width:'.$Largura.';">
                        <div class="editor_popup_titulo"  style="cursor:pointer" onClick=\'window.open("'.$_SESSION['UrlBase'].$Pasta.'/editor.tpl.php?Nome='.$Nome.'&Ferramentas='.$Ferramentas.'&Pasta='.$Pasta.'", "'.$Nome.'", "status=no, scrollbars=yes, width=680, height=520")\'>
    ...
    {
        if($CFG['ProcessarComo'] == '') $CFG['ProcessarComo'] = "Inteiro";
    ...
    {
        if($CFG['ProcessarComo'] == '') $CFG['ProcessarComo'] = "Float";
    ...
    {
        if($CFG['ProcessarComo'] == '') $CFG['ProcessarComo'] = "Data";
    ...
    public function listBox($CFG, $Obrigatorio = true)
    {
        if($CFG['ProcessarComo'] == '') $CFG['ProcessarComo'] = "TextoInteiro";
        ...
        if($this->ProcessarHtml)
        {
            ...
            if(!empty($AutoSelect ))      $Sql = "SELECT $AutoSelect FROM $Tabela $Condicao";
            elseif (!empty($FullSelect))  $Sql = $FullSelect;
            else                          $Sql = "SELECT $CampoCod, $CampoDesc FROM $Tabela $Condicao";
    ...
    public function listaVetor($CFG, $Obrigatorio = true)
    {
        ...
        {
            //Verifica Inicio
            if($Inicio != "") $Campo = ($Inicio === true) ? "<option value=\"\">Selecione...</option>" : "<option value=\"\">$Inicio</option>";
            ...
            if($Conteiner != '')
            {
                $Retorno = "<div id=\"$Conteiner\">".$this->htmlSelect($CFG, $Campo)."</div>";
            }
            else
            {
    ...
    public function inputUF($Campo, $ValorCampo, $SetUF, $Adicional = NULL, $Inicio = NULL)
    {
        if($this->ProcessarHtml)
        {
            if($ValorCampo != '') $SetUF = $ValorCampo;
            ...
            $StringInput .= '<option value="AC"'; if($SetUF == "AC") $StringInput .= $S; $StringInput .='>AC</option>';
            $StringInput .= '<option value="AL"'; if($SetUF == "AL") $StringInput .= $S; $StringInput .='>AL</option>';
            $StringInput .= '<option value="AM"'; if($SetUF == "AM") $StringInput .= $S; $StringInput .='>AM</option>';
            $StringInput .= '<option value="AP"'; if($SetUF == "AP") $StringInput .= $S; $StringInput .='>AP</option>';
            $StringInput .= '<option value="BA"'; if($SetUF == "BA") $StringInput .= $S; $StringInput .='>BA</option>';
            $StringInput .= '<option value="CE"'; if($SetUF == "CE") $StringInput .= $S; $StringInput .='>CE</option>';
            $StringInput .= '<option value="DF"'; if($SetUF == "DF") $StringInput .= $S; $StringInput .='>DF</option>';
            $StringInput .= '<option value="ES"'; if($SetUF == "ES") $StringInput .= $S; $StringInput .='>ES</option>';
            $StringInput .= '<option value="GO"'; if($SetUF == "GO") $StringInput .= $S; $StringInput .='>GO</option>';
            $StringInput .= '<option value="MA"'; if($SetUF == "MA") $StringInput .= $S; $StringInput .='>MA</option>';
            $StringInput .= '<option value="MG"'; if($SetUF == "MG") $StringInput .= $S; $StringInput .='>MG</option>';
            $StringInput .= '<option value="MS"'; if($SetUF == "MS") $StringInput .= $S; $StringInput .='>MS</option>';
            $StringInput .= '<option value="MT"'; if($SetUF == "MT") $StringInput .= $S; $StringInput .='>MT</option>';
            $StringInput .= '<option value="PA"'; if($SetUF == "PA") $StringInput .= $S; $StringInput .='>PA</option>';
            $StringInput .= '<option value="PB"'; if($SetUF == "PB") $StringInput .= $S; $StringInput .='>PB</option>';
            $StringInput .= '<option value="PE"'; if($SetUF == "PE") $StringInput .= $S; $StringInput .='>PE</option>';
            $StringInput .= '<option value="PI"'; if($SetUF == "PI") $StringInput .= $S; $StringInput .='>PI</option>';
            $StringInput .= '<option value="PR"'; if($SetUF == "PR") $StringInput .= $S; $StringInput .='>PR</option>';
            $StringInput .= '<option value="RJ"'; if($SetUF == "RJ") $StringInput .= $S; $StringInput .='>RJ</option>';
            $StringInput .= '<option value="RN"'; if($SetUF == "RN") $StringInput .= $S; $StringInput .='>RN</option>';
            $StringInput .= '<option value="RO"'; if($SetUF == "RO") $StringInput .= $S; $StringInput .='>RO</option>';
            $StringInput .= '<option value="RR"'; if($SetUF == "RR") $StringInput .= $S; $StringInput .='>RR</option>';
            $StringInput .= '<option value="RS"'; if($SetUF == "RS") $StringInput .= $S; $StringInput .='>RS</option>';
            $StringInput .= '<option value="SC"'; if($SetUF == "SC") $StringInput .= $S; $StringInput .='>SC</option>';
            $StringInput .= '<option value="SE"'; if($SetUF == "SE") $StringInput .= $S; $StringInput .='>SE</option>';
            $StringInput .= '<option value="SP"'; if($SetUF == "SP") $StringInput .= $S; $StringInput .='>SP</option>';
            $StringInput .= '<option value="TO"'; if($SetUF == "TO") $StringInput .= $S; $StringInput .='>TO</option>';
            $StringInput .= '</select>';

            return $StringInput;
        }

        function inputMes($Campo, $ValorCampo, $SetMes)
        {
            if($ValorCampo != '') $SetMes = $ValorCampo;
            ...
            $StringInput .= '<option value="1"' ; if($SetMes == "1") 	$StringInput .= $S; $StringInput .='>Janeiro</option>';
            $StringInput .= '<option value="2"' ; if($SetMes == "2") 	$StringInput .= $S; $StringInput .='>Fevereiro</option>';
            $StringInput .= '<option value="3"' ; if($SetMes == "3") 	$StringInput .= $S; $StringInput .='>Março</option>';
            $StringInput .= '<option value="4"' ; if($SetMes == "4") 	$StringInput .= $S; $StringInput .='>Abril</option>';
            $StringInput .= '<option value="5"' ; if($SetMes == "5")	$StringInput .= $S; $StringInput .='>Maio</option>';
            $StringInput .= '<option value="6"' ; if($SetMes == "6") 	$StringInput .= $S; $StringInput .='>Junho</option>';
            $StringInput .= '<option value="7"' ; if($SetMes == "7") 	$StringInput .= $S; $StringInput .='>Julho</option>';
            $StringInput .= '<option value="8"' ; if($SetMes == "8") 	$StringInput .= $S; $StringInput .='>Agosto</option>';
            $StringInput .= '<option value="9"' ; if($SetMes == "9") 	$StringInput .= $S; $StringInput .='>Setembro</option>';
            $StringInput .= '<option value="10"'; if($SetMes == "10") 	$StringInput .= $S; $StringInput .='>Outubro</option>';
            $StringInput .= '<option value="11"'; if($SetMes == "11")   $StringInput .= $S; $StringInput .='>Novembro</option>';
            $StringInput .= '<option value="12"'; if($SetMes == "12")   $StringInput .= $S; $StringInput .='>Dezembro</option>';
            $StringInput .= '</select>';
    ...
    {        
        ...
        if(is_array($AddFiltro)) $ArrayTipos += $AddFiltro;
