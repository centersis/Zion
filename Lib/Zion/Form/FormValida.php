                        ...
                        switch ($acao) {
                            case 'CPF':
                                if ($this->geral->validaCPF($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é um CPF válido!");
                                }
                                break;

                            case 'CNPJ':
                                if ($this->geral->validaCNPJ($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é um CNPJ válido!");
                                }
                                break;

                            case 'CEP':
                                if ($this->geral->validaCEP($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é um CEP válido!");
                                }
                                break;

                            case 'FLOAT':
                                if ($this->numero->isFloat($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é um float válido!");
                                }
                                break;

                            case 'DATA':
                                if ($this->data->validaData($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é uma data válida!");
                                }
                                break;

                            case 'HORA':
                                if ($this->data->validaHora($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é uma hora válida!");
                                }
                                break;

                            case 'NUMBER':
                                if (\is_numeric($userValue) === false) {
                                    throw new InvalidArgumentException($identifica . ": O valor informado não é um numero válido!");
                                }
                                break;

//                            case 'ESCOLHA':
//                                if (empty($userValue) and $formInput->getObrigatorio() === true) {
//                                    throw new InvalidArgumentException($identifica . ": Você deve selecionar uma das opções!");
//                                }
//                                break;
                        }
                    }
                    ...
                    if ($acao == 'CHOSEN' || $acao == 'ESCOLHA') {
                        ...
                        if (\is_array($userValue)) {
                            ...
                            if (\count($userValue) < 1 and $formInput->getObrigatorio() === true) {
                                throw new InvalidArgumentException($identifica . ": Você deve selecionar uma ou mais opções!");
                            }

                            if (!$dependencia) {
                                foreach ($userValue as $valorSelecionado) {
                                    if (!\array_key_exists($valorSelecionado, $valoresReais)) {
                                        throw new InvalidArgumentException($identifica . ": O valor recuperado não corresponde a um valor válido!");
                                    }
                                }
                            }
                        } else {
                            ...
                            if ($userValue == '' and $formInput->getObrigatorio() === true) {
                                throw new InvalidArgumentException($identifica . ": selecione uma ou mais opções!");
                            }
                ...
                case 'selecaoMinima':
                    ...
                    if (!empty($val) and ( !empty($userValue) or $obrigatorio === true)) {
                        if (\count($userValue) < $val) {
                            throw new InvalidArgumentException($identifica . ": Você deve selecionar no mínimo " . $val . " # opções! ");
                        }
                    }
                    break;
                ...
                case 'selecaoMaxima':
                    ...
                    if (!empty($val) and ( \is_array($userValue) or $obrigatorio === true)) {
                        if (\count($userValue) > $val) {
                            throw new InvalidArgumentException($identifica . ": Você deve selecionar no máximo " . $val . " opções!");
                        }
                    }
                    break;
                ...
                case 'obrigatorio':
                    if ($formInput->getObrigatorio() === true) {
                        if (empty($userValue) and $userValue <> 0) {
                            throw new InvalidArgumentException($identifica . ": Nenhum valor informado!");
                        }
                    }
                    break;
                ...
                case 'minimoCaracteres':
                    ...
                    if (!empty($val) and ( !empty($userValue) or $obrigatorio === true)) {
                        if ($this->texto->verificaMinimoCaracteres($val, $userValue) === false) {
                            throw new InvalidArgumentException($identifica . ": O valor informado é menor que o tamanho mínimo solicitado de " . $val . " caracteres!");
                        }
                    }
                    break;
                ...
                case 'maximoCaracteres':
                    ...
                    if (!empty($val) and ( !empty($userValue) or $obrigatorio === true)) {
                        if ($this->texto->verificaMaximoCaracteres($val, $userValue) === false) {
                            throw new InvalidArgumentException($identifica . ": O valor informado excede o tamanho máximo permitido de " . $val . " caracteres!");
                        }
                    }
                    break;
                ...
                case 'valorMinimo':
                    ...
                    if (\is_numeric($val) and ( !empty($userValue) or $obrigatorio === true)) {
                        if ($this->numero->verificaValorMinimo($val, $userValue) === false) {
                            throw new InvalidArgumentException($identifica . ": O valor informado não pode ser menor que " . $val . "!");
                        }
                    }
                    break;
                ...
                case 'valorMaximo':
                    ...
                    if (\is_numeric($val) and ( !empty($userValue) or $obrigatorio === true)) {
                        if ($this->numero->verificaValorMaximo($val, $userValue) === false) {
                            throw new InvalidArgumentException($identifica . ": O valor informado não pode ser maior que " . $val . "!");
                        }
                    }
                    break;
