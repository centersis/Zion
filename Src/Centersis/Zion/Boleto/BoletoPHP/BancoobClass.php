...
class BancoobClass
{
    ...
    public function getDadosBoleto($dadosParcela, $parametrizacao)
    {
        ...
        $valorParcela = number_format($this->tratamento->numero()->floatBanco($dadosParcela["fnc_parcela_valor"]), 2, ',', '');

        if ((isset($parametrizacao['fnc_convenio_dispensa_valor']) and $parametrizacao['fnc_convenio_dispensa_valor'] == 'S') or $valorParcela == '0,01') {
        ...
        $modalidadecobranca = '0' . $parametrizacao["fnc_convenio_carteira"];
        $numeroparcela = '001';

        $convenio = $this->formataNumero($parametrizacao["fnc_convenio_numero"], 7, 0);

        $agencia_codigo = $agencia . " / " . $convenio;

        $nossonumero = $this->formataNumero($dadosParcela["fnc_parcela_nosso_numero"], 8, 0);
        $campolivre = "$modalidadecobranca$convenio$nossonumero$numeroparcela";

        $dv = $this->modulo11("$codigobanco$nummoeda$fatorVencimento$valor$carteira$agencia$campolivre");
        $linha = "$codigobanco$nummoeda$dv$fatorVencimento$valor$carteira$agencia$campolivre";
    ...
    private function dataParaDias($year, $month, $day)
    {
