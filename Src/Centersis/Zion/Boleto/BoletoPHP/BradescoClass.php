...
class BradescoClass
{
    ...
    public function getDadosBoleto($dadosParcela, $parametrizacao)
    {
        ...
        $nnumComDv = $this->formataNumero($dadosParcela["fnc_parcela_nosso_numero"], 12, 0);

        //nosso número (sem dv) é 11 digitos
        $nnumSemDv = substr($nnumComDv, 0, - 1);

        $nnum = $carteira . $nnumSemDv;
        ...
        //conta
        $conta = $this->formataNumero($parametrizacao["fnc_convenio_conta"], 7, 0);
        $conta_dv = $this->formataNumero($parametrizacao["fnc_convenio_conta_dv"], 1, 0);

        $dv = $this->digitoVerificadorBarra("$codigobanco$nummoeda$fatorVencimento$valor$agencia$nnum$conta" . '0');

        $linha = "$codigobanco$nummoeda$dv$fatorVencimento$valor$agencia$nnum$conta" . "0";

        $nnP1 = $carteira;
        $nnP2 = $nnumSemDv;
        $nnP3 = substr($nnumComDv, -1);

        $nossonumero = $nnP1 . '/' . $nnP2 . '-' . $nnP3;
        $agenciaCodigo = $agencia . "-" . $agenciaDv . " / " . $conta . "-" . $conta_dv;
