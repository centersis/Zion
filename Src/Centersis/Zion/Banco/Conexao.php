...
class Conexao {
    ...
    private function __construct($banco, $host = '', $usuario = '', $senha = '', $driver = '') {
        ...
        self::$link[$banco] = DriverManager::getConnection($connectionParams);
    }
    ...
    public static function conectar($banco = 'padrao') {
        $bancoMinusculo = empty($banco) ? 'padrao' : strtolower($banco);

        if (!isset(self::$instancia[$bancoMinusculo])) {
            self::$instancia[$bancoMinusculo] = new Conexao($bancoMinusculo);
        }
        ...
        return self::$instancia[$bancoMinusculo];
    }
...
//                }
            ...
            if ($sql->getType() === 0) { //0 = SELECT               
                ...
                $resultSet = $sql->executeStatement();
    ...
    private function isQueryBuilder($qb) {
        return (get_class($qb) === 'Doctrine\DBAL\Query\QueryBuilder') ? true : false;
    }
