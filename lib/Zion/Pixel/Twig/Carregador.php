...
class Carregador {
    ...
    public function __construct($namespace = '', $paths = []) {
        ...
        if ($namespace) {
            ...
            $dirnamespaceAntigo = $this->interpretaNamespace($namespace) . '/Tema/Vendor/' . SIS_VENDOR_TEMPLATE . '/views';

            if ($this->dir->eDiretorio($dirnamespaceAntigo)) {
                $this->caminhos[] = $dirnamespaceAntigo;
            }

            $dirnamespaceNovo = $this->interpretaNamespace($namespace) . '/Views';
            
            if ($this->dir->eDiretorio($dirnamespaceNovo)) {
                $this->caminhos[] = $dirnamespaceNovo;
            }
        }
        ...
        $urlBase = new \Twig\TwigFunction('urlBase', function ($url) {
                    return \SIS_URL_BASE . $url;
                });

        $urlBaseTema = new \Twig\TwigFunction('urlBaseTema', function ($url) {
                    return SIS_URL_DEFAULT_BASE . 'Tema/Vendor/' . SIS_VENDOR_TEMPLATE . '/' . $url;
                });

        $urlFramework = new \Twig\TwigFunction('urlFramework', function ($url) {
                    return \SIS_URL_FM_BASE . $url;
                });

        $urlBaseSite = new \Twig\TwigFunction('urlBaseSite', function ($url) {
                    return \SIS_URL_BASE_SITE . $url;
                });

        $urlBaseStorage = new \Twig\TwigFunction('urlBaseStorage', function ($url) {
