/*********************************************
**                                          **
**   Funções de Validação Por Pablo Vanni   **
**                                          **
*********************************************/
...
function vazio(Valor, Identifica){
	$Valor = $.trim(Valor);
	if($Valor == '') { alert("O Campo "+Identifica+" não pode ser vazio!"); return false; } return true;
}
...
function tMaximo(Valor, Max, Identifica){
	if(Max > 0 ) { if(Valor.length > Max){ alert("O Campo "+Identifica+" deve ter no máximo "+Max+" Caracteres"); return false; }} return true;
}
...
function vMinimoData(Valor, VMin, DataAtual,  Identifica){
	if(VMin > 0 )  { if(Valor < VMin){ alert("O Campo "+Identifica+" não pode conter data menor que "+DataAtual); return false; }} return true;
}

//Valor Máximo
function vMaximo(Valor, VMax, Identifica){
	if(VMax > 0 )  { if(Valor > VMax){ alert("O Campo "+Identifica+" deve ter valor máximo de "+VMax); return false; }} return true;
}

//Valor Máximo Data
function vMaximoData(Valor, VMax, DataAtual, Identifica){
	if(VMax > 0 )  { if(Valor > VMax){ alert("O Campo "+Identifica+" não pode conter data maior que "+DataAtual); return false; }} return true;
}
...
function vInteiro(Valor, Identifica){
	if(isNaN(Valor))  { alert("O Campo "+Identifica+" não é um número inteiro!"); return false; } return true;	
}
...
function validaData(Valor, VMax, VMin, Identifica){
	VMax = interpretaEval(VMax); VMin = interpretaEval(VMin); var ExpReg = /^((0[1-9]|[12]\d)\/(0[1-9]|1[0-2])|30\/(0[13-9]|1[0-2])|31\/(0[13578]|1[02]))\/\d{4}$/; if(!vazio(Valor, Identifica)) return false; if(!ExpReg.test(Valor)) { alert("O Campo "+Identifica+" não possui uma data válida"); return false; } if(VMin != 0) { if(!dataMinima(Valor, VMin, Identifica)) return false; } if(VMax != 0) { if(!dataMaxima(Valor, VMax, Identifica)) return false; } return true;
}
...
function validaCPF(Valor, Identifica){
	if(!vazio(Valor, Identifica)) { alert("CPF inválido!"); return false; }  if(Valor.length != 14 ) { alert("O Campo "+Identifica+" deve ter 14 Caracteres"); return false; }try{ var Vet1  = Valor.split(".");	var Vet2  = Vet1[2].split("-");	Valor = Vet1[0]+Vet1[1]+Vet2[0]+Vet2[1]; } catch(e) { alert("CPF inválido!"); return false; }if(cpf(Valor)) return true; else { alert("CPF inválido!"); return false; }
}

//Email
function validaEmail(email){
	if(!vemail(email)) { alert('Email Inválido!'); return false; } else return true;
}

//Fone
function validaFone(Fone){
	if(Fone.length < 1){ alert('Telefone Inválido'); return false;} else { return true; }
}
