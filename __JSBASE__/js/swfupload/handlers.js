...
function uploadStart(fileObj) {
	try {
		...
		var progress = new FileProgress(fileObj, this.customSettings.progressTarget);
		progress.SetStatus("Carregando Arquivo...");
		progress.ToggleCancel(true, this);
	}
	catch (ex) {}
	...
	try {
		...
		switch(error_code) {
			case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
				progress.SetStatus("Erro ao Carregar o Arquivo: " + message+" "+error_code);
...
function FileProgress(fileObj, target_id) 
{
	...
	if (!this.fileProgressWrapper) 
	{
		...
		progressCancel.href = "javascript:cancelQueue("+fileObj+")";
...
FileProgress.prototype.SetError = function() 
{
	...
	var oSelf = this;
	setTimeout(function() { oSelf.Disappear(); }, 50);
};
...
FileProgress.prototype.SetCancelled = function() 
{
	...
	var oSelf = this;
	setTimeout(function() { oSelf.Disappear(); }, 20);
};
